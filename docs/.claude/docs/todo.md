...
- [x] **Экран 5: Выбор услуги и даты** `BookServiceScreen`
  - [x] Вёрстка из файла дизайна
  - [x] `RadioGroup` выбор услуги из списка
  - [x] Горизонтальный `LazyRow` выбора даты (ближайшие 14 дней)
  - [x] Кнопка «Посмотреть слоты» → `SelectSlotScreen`

- [x] **Экран 6: Выбор слота** `SelectSlotScreen`
  - [x] Вёрстка из файла дизайна
  - [x] GET `/api/organizations/{id}/available-slots?serviceId=&date=`
  - [x] `FlowRow` / сетка временных слотов (сотрудник + время)
  - [x] Tap → выделить выбранный слот
  - [x] Кнопка «Продолжить» → `ConfirmBookingScreen`
  - [x] Пустое состояние: «Нет свободных слотов»

- [x] **Экран 7: Подтверждение** `ConfirmBookingScreen`
  - [x] Вёрстка из файла дизайна
  - [x] Итог: услуга, сотрудник, дата/время, цена
  - [x] `OutlinedTextField` комментарий (опц.)
  - [x] Кнопка «Подтвердить запись» → POST `/api/bookings`
  - [x] При успехе → `BookingDetailScreen(id)`

---

### Фаза 4 — Мои записи

- [x] **Экран 8: Список записей** `BookingsScreen` (tab: Записи)
  - [x] Вёрстка из файла дизайна
  - [x] GET `/api/bookings/my`
  - [x] Горизонтальные чипы-фильтры: Все / Активные / Завершённые / Отменённые
  - [x] `LazyColumn`: услуга, организация, дата/время, `StatusBadge`
  - [x] `SwipeRefresh`
  - [x] Tap → `BookingDetailScreen`
  - [x] Пустое состояние

- [x] **Экран 9: Детали записи** `BookingDetailScreen`
  - [x] Вёрстка из файла дизайна
  - [x] GET `/api/bookings/{id}`
  - [x] Карточка: организация, услуга, сотрудник, дата/время, статус
  - [ ] Таймлайн истории статусов (требует endpoint истории — пропущено)
  - [x] Кнопка «Показать QR-код» (статус `confirmed`) → `QrCodeScreen`
  - [x] Кнопка «Отменить запись» (статус `created`/`confirmed`) → AlertDialog + POST `/api/bookings/{id}/cancel`

- [x] **Экран 10: QR-код** `QrCodeScreen`
  - [x] Вёрстка из файла дизайна
  - [x] GET `/api/bookings/{id}/qr` → токен
  - [x] ZXing `BarcodeEncoder` → `Bitmap` QR-кода → `Image` (Compose)
  - [x] QR крупный, на большую часть экрана, белый фон
  - [x] Имя, услуга, дата/время под QR
  - [x] Кнопка «Назад»
